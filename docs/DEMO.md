...
4. Dependencies install from `requirements.txt` (Python **3.11+** required for current Streamlit).
5. Secrets: `GROQ_API_KEY`, `TAVILY_API_KEY`, `LLM_PROVIDER=groq`, `USE_MOCK_WHEN_NO_KEYS=false`.
6. Paste URL in deck: **Prototype link**.

If deploy fails with “streamlit requires Python >=3.10”, ensure Python **3.11** is selected and push `.python-version`, `requirements.txt`, and `pyproject.toml`.
